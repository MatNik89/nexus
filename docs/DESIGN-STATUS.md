...
| 3 | Sandbox feasibility → platform odluka | **AŽURIRANO 2026-09-03 (HARDQ D1): bwrap = P0 ENFORCED backend** (korisnikova odluka; isti hostile-suite invarijant, fail-closed, atestiran); native helper (no_new_privs→Landlock→per-arch-seccomp→execveat, bez pre-exec prozora) = **P1 hardening put**. Probe OPEN = hostile-conformance suite protiv bwrapa na stvarnom deployment hostu. Win/macOS UNAVAILABLE+high-risk BLOKIRAN, nula weaker-fallback; njihovi probeovi (W0-W10/M0-M9) odgođeni s platformama. | codex+korisnik | prijašnji native-first stav zamijenjen D1 odlukom |
