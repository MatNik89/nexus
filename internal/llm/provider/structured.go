...
// ReAsk is one additional PHYSICAL provider attempt. It always carries a
// fresh S7 AttemptGrant (SPEC P0.2: every physical attempt presents one) —
// the extractor issues and accounts it; adapters never retry on their own.
...
func Extract[T any](ctx context.Context, e *Extractor, class PayloadClass, raw []byte,
	...
	// GENERAL: one re-ask, a NEW physical attempt under S7 accounting.
	var reaskRaw []byte
	if reask != nil {
		g, gerr := e.auth.Issue(op, target)
		if gerr == nil {
			if cerr := e.auth.Consume(g); cerr == nil {
	...
	// Salvage: re-ask output first, then the original.
