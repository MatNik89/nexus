...
// real sink in T16/T17; tests pass capturing fakes).
type AuditSink interface {
	Record(event string, call contracts.ToolCall)
}
...
// RunTool executes ONE tool call: S6.0 Decide → S6.9 Before → grant
// consume (s7, BEFORE execute) → sealed-kind executor → S6.9 After/OnError
// → s7 outcome. Every refusal returns an explicitly empty ToolResult.
func (p *EffectPath) RunTool(ctx context.Context, call contracts.ToolCall, grant s7min.Grant) (contracts.ToolResult, error) {
	...
	case DecisionAsk:
		if p.pep.mode == ModeYolo {
			// HARDQ F2: confirmations only — journaled, never silent.
			p.pep.audit.Record("ALLOWED_BY_YOLO", call)
	...
	case contracts.ExecProcess:
		exec = p.sbproc
	default:
		return contracts.ToolResult{}, p.mw.OnError(ctx,
	...
	if err != nil {
		// Executor error: AfterTool and output are SKIPPED; the attempt is
		// terminal (no-retry P0) and the result explicitly empty.
		oe := p.mw.OnError(ctx, err)
		if rerr := p.grants.Report(grant.OperationID, s7min.OutcomeFailedTerminal); rerr != nil {
			oe = errors.Join(oe, rerr)
		}
		return contracts.ToolResult{}, oe
	}
	// 6) S6.9 order: After — a veto goes through OnError; an effectful
	// result without a valid receipt parks UNKNOWN (reconcile, no blind
	// retry — N3-C02/N4-C03).
	if verr := p.mw.AfterTool(ctx, call, out); verr != nil {
		oe := p.mw.OnError(ctx, verr)
		outcome := s7min.OutcomeUnknown
		if ClassifyEffectPhase(call, out) == contracts.PhaseBeforeCommit {
			outcome = s7min.OutcomeFailedTerminal // nothing durable could exist
		}
		if rerr := p.grants.Report(grant.OperationID, outcome); rerr != nil {
			oe = errors.Join(oe, rerr)
		}
		return contracts.ToolResult{}, oe
	}
	// 7) s7 records the outcome (sole retry owner; P0 retries nothing).
	outcome := s7min.OutcomeFailedTerminal
	switch out.Status {
	case contracts.ResultSucceeded:
		outcome = s7min.OutcomeSucceeded
	case contracts.ResultUnknown:
		outcome = s7min.OutcomeUnknown
	}
	if rerr := p.grants.Report(grant.OperationID, outcome); rerr != nil {
		return contracts.ToolResult{}, p.mw.OnError(ctx, rerr)
	}
	return out, nil
}
